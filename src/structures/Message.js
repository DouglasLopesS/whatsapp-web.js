...
class Message extends Base {
    ...
    async delete(everyone, clearMedia = true) {
        await this.client.pupPage.evaluate(async (msgId, everyone, clearMedia) => {
            ...
            const canRevoke =
                window.require('WAWebMsgActionCapability').canSenderRevokeMsg(msg) ||
                window.require('WAWebMsgActionCapability').canAdminRevokeMsg(msg);
    ...
    async edit(content, options = {}) {
        ...
        const messageEdit = await this.client.pupPage.evaluate(async (msgId, message, options) => {
            ...
            let canEdit = window.require('WAWebMsgActionCapability').canEditText(msg) ||
                window.require('WAWebMsgActionCapability').canEditCaption(msg);
            if (canEdit) {
